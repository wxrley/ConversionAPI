# 🔄 Conversion API ☕
Uma API REST que oferece serviços de conversão para moedas e unidades de medida.

## 💡 Sobre o projeto
Este projeto foi desenvolvido para demonstrar conceitos do **Spring Boot**. \
O **ConversionAPI** é uma API de conversão que conta com tratamento de exceções, validação de entradas e uma documentação interativa.

## ⚙️ Funcionalidades
- **Conversão Monetária:** Realiza câmbio entre USD, BRL e EUR utilizando taxas pré-definidas.
- **Conversão de Temperaturas:** Transforma valores entre Celsius (C) e Fahrenheit (F).
- **Conversão de Distâncias:** Calcula equivalências entre Quilômetros (KM) e Milhas (MI).
- **Validação de Dados:** Impede o processamento de moedas/unidades não suportadas e valores negativos (quando aplicável).
- **Segurança Operacional:** Validação de taxas inexistentes e parâmetros inválidos.
- **Documentação Automática:** Interface visual via Swagger para testes imediatos.

## 🧩 Tecnologias Utilizadas
- Java 21
- Spring Boot
- Maven
- Swagger/OpenAPI

## 🛠️ Instalação
**1.** Verifique se o **Java** e o **Maven** estão instalados na sua máquina:
>Para confirmar, execute no terminal:
>```bash
>java -version
>mvn -version
>```
> Caso precise, baixe o [JDK](https://www.oracle.com/java/technologies/downloads) ou [Maven](https://maven.apache.org/download.cgi).

**2.** Baixe ou clone este repositório para o seu computador:
>```bash
> git clone https://github.com/wxrley/ConversionAPI.git
>```

## 🚀 Execução
#### Opção 1 — Via Terminal (Maven)
**1.** Entre na pasta raiz do projeto e execute o comando para iniciar a aplicação:
>```bash
>mvn spring-boot:run
>```
**2.** O console deve exibir o log do Spring indicando que o servidor subiu.

#### Opção 2 — Via IDE (IntelliJ, Eclipse, VS Code, etc.)
**1.** Abra a pasta do projeto na sua IDE preferida. \
**2.** Abra o arquivo `ConversionAPIApplication.java` e clique em **Run**.

## 🧪 Como Testar
Para facilitar o teste das operações, o projeto utiliza o **Swagger (OpenAPI).** Isso permite que você visualize e execute todos os endpoints diretamente pelo navegador, de forma totalmente interativa.

**1.** Certifique-se de que a aplicação está rodando. \
**2.** Acesse o link oficial da documentação:
> http://localhost:8080/swagger-ui/index.html

**3.** Exemplos de Endpoints:
- Escolha uma das operações (ex: `/convert/currency`).
- Clique no botão **"Try it out"**.
- Preencha os campos (como `from`, `to` e `amount`).
- Clique no botão azul **"Execute"**.
- O resultado aparecerá logo abaixo, na seção **"Response body"**.

> [!NOTE]
> O Swagger também valida automaticamente as entradas e exibe as mensagens de erro caso você tente realizar uma operação inválida (como moedas não suportadas).

## 👨‍💻 Autor
**Wxrley** — só mais um dev latino americano 💪
